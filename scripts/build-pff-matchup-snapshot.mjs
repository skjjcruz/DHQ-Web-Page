...
async function main() {
    ...
    for (const t of teams) {
        try {
            ...
            depth[t.sleeper] = (r.rows || []).map(p => ({
                ...
                g: num(p.grade), gr: num(p.gradeRank), of: num(p.gradeRankOf), sn: num(p.snapCounts), sp: num(p.snapPct), st: p.status || '',
            })).filter(p => p.n);
    ...
    const REPORTS = ['passing', 'rushing', 'receiving', 'defense', 'pass-rush', 'coverage', 'run-defense', 'field-goals', 'pass-blocking', 'run-blocking'];
    const players = {};
    const reportCols = {};
    ...
    const js = '// Generated by scripts/build-pff-matchup-snapshot.mjs — do not edit by hand.\n'
        ...
        + 'window.DhqPffMatchup = ' + JSON.stringify(snapshot) + ';\n'
        + 'if (typeof module !== "undefined" && module.exports) module.exports = window.DhqPffMatchup;\n';
